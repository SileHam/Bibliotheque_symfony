...
class LivresFixtures extends Fixture implements DependentFixtureInterface
{
    public function load(ObjectManager $manager): void
    {
        ...
        for($i = 1; $i <= 50; $i++){
            ...
            // Ajouter 1 ou 2 auteurs
            for($j = 1; $j <= $faker->numberBetween(1, 2); $j++){
                $book->addAuteur($this->getReference('auteur_'.$faker->numberBetween(1, 20), Auteur::class));
            }
            
            // Ajouter 1 ou 2 genres
            for($j = 1; $j <= $faker->numberBetween(1, 2); $j++){
                $book->addGenre($this->getReference('genre_'.$faker->numberBetween(1, 10), Genre::class));
            }
